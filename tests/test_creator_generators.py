...
class TestWorldGenerator:
    async def test_generates_world_data(self):
        ...
        result = await generate_world(_proposal(), client, creator_config())
    ...
    async def test_raises_after_max_retries(self):
        client = make_mock_client(["bad yaml {{{", "still bad {{{"])
        with pytest.raises(YAMLParseError):
            await generate_world(_proposal(), client, creator_config())
