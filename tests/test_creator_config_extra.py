...
The existing tests for load_creator_config() happen to go through the
settings-file path when SETTINGS_FILE.exists() is True. These tests
monkeypatch SETTINGS_FILE.exists() to False so we exercise lines 109-124
(the original env-var-only fallback).
"""
...
from theact.creator.config import _GAMEPLAY_MODEL, load_creator_config
...
class TestEnvVarOnlyFallback:
    ...
    def test_defaults_to_gameplay_model_with_warning(self, monkeypatch):
        """When no model is set, defaults to gameplay model and warns."""
        ...
        assert config.model == _GAMEPLAY_MODEL
    ...
    def test_no_warning_for_large_model(self, monkeypatch):
        """No warning emitted when a large model is explicitly configured."""
